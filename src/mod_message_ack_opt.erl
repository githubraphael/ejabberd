...
bounce_groupchat(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, bounce_groupchat).
...
cache_life_time(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, cache_life_time).
...
cache_size(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, cache_size).

-spec db_type(gen_mod:opts() | global | binary()) -> atom().
db_type(Opts) when is_map(Opts) ->
    gen_mod:get_opt(db_type, Opts);
db_type(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, db_type).
...
store_empty_body(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, store_empty_body).
...
store_groupchat(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, store_groupchat).

-spec use_cache(gen_mod:opts() | global | binary()) -> boolean().
use_cache(Opts) when is_map(Opts) ->
    gen_mod:get_opt(use_cache, Opts);
use_cache(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, use_cache).
...
use_mam_for_storage(Host) ->
    gen_mod:get_module_opt(Host, mod_message_ack, use_mam_for_storage).
