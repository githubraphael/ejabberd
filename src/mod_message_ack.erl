...
init_cache(Mod, Host, Opts) ->
  CacheOpts = [{max_size, mod_message_ack_opt:cache_size(Opts)},
    {life_time, mod_message_ack_opt:cache_life_time(Opts)},
    {cache_missed, false}],
...
use_cache(Mod, Host) ->
  case erlang:function_exported(Mod, use_cache, 1) of
    ...
    false -> mod_message_ack_opt:use_cache(Host)
  end.
...
get_max_user_messages(User, Server) ->
  Access = mod_message_ack_opt:access_max_user_messages(Server),
        ...
        _ ->
          case check_store_hint(Packet) of
            ...
            none ->
              Store = case Type of
                        groupchat ->
                          mod_message_ack_opt:store_groupchat(LServer);
                        headline ->
                          false;
                        _ ->
                          true
              ...
              case {misc:get_mucsub_event_type(Packet), Store,
                mod_message_ack_opt:store_empty_body(LServer)} of
...
check_if_message_should_be_bounced(Packet) ->
  case Packet of
    #message{type = groupchat, to = #jid{lserver = LServer}} ->
      mod_message_ack_opt:bounce_groupchat(LServer);
...
use_mam_for_user(_User, Server) ->
  mod_message_ack_opt:use_mam_for_storage(Server).
